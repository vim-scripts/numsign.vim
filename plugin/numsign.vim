...
fun! s:InitVariable()
    ...
    if !exists("b:sign_working_id")
        let b:sign_working_id = 0	"it's jumping sign notation, 0 fullfill nextid is 1, previous id is MAX-1
    endif
...
fun! s:PlaceSign()
	let ln = line(".")
	let sign_place_id = GetNextPlaceID()
	call SignDefine(sign_place_id)
	exe 'sign place ' . sign_place_id . ' line=' . ln . ' name=SignBlue' . sign_place_id . ' buffer=' . winbufnr(0)
	call GetAllSigns()
	call s:SyncData()
	"debug
	"let g:allSignsSortByID=b:allSignsSortByID
endfun
" ---------------------------------------------------------------------
" ClearSign: {{{1	PRIVATE
...
endfun
" ---------------------------------------------------------------------
"  GetNextPlaceID {{{ 1 private
fun! GetNextPlaceID()
	let size = len(b:allSignsSortByID)
	if(size == 0) | return 1 | endif		" sign place id begin from 1
	return b:allSignsSortByID[size-1][0] + 1	" the next place ID
endfun
